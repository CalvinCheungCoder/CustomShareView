...
-(void)setShareBtn{
    ...
    NSArray *arr = [NSArray arrayWithObjects:@"新浪分享",@"微信分享",@"QQ分享",@"淘宝分享",@"生日管家分享", nil];
    for (int i = 0; i < 5; i ++) {
        
        UIButton *btn = [[UIButton alloc]init];
        btn.frame = CGRectMake(50, 80 * (i + 1), [UIScreen mainScreen].bounds.size.width - 100, 50);
...
- (void)addWeiboShareView {
    ...
    //自定义头部
    UIView *headerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, Width, 36)];
    ...
    //实例化
    ShareView *shareView = [[ShareView alloc] initWithFrame:CGRectMake(0, 0, Width, Height)];
...
- (void)addGuanjiaShareView {
    ...
    UIView *headerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, Width, 54)];
    headerView.backgroundColor = [UIColor clearColor];
